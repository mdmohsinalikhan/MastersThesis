...
unsigned long long int* generateSampleWithoutreplacement()
{
	...
	//to to sampling without replacement we have to sor it. that is uncomment the next line.	
	...
	for(i = 0; i < samplesize; i++)
	{
		//to do sampling without replacement along with the above sorting, the below index needs to be changed from 0 to 1
		sample_plaintext_set[i] = sample_plaintext_set_local[i][0];
