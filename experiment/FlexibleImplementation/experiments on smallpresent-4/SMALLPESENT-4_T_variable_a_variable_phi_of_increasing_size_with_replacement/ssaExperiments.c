...
void preparePythonScriptForPlotting(long double ***T_variable_phi_variable_a)
{
	...
	for(round = 1;round <= ROUND; round++)
	{
		...
		for(j = 5; j <= 4*n - NOFIB; j++)
		{
			...
			fprintf(f,"mu_%.0f = %llu + %.0f*xxxx\n",pow(2,j),total_outputs-1,pow(2,j));
			fprintf(f,"sigma_%.0f = math.sqrt((2.0/%llu)*(%llu+%0.f*xxxx)**2)\n",pow(2,j),total_outputs-1,total_outputs-1,pow(2,j));
			fprintf(f,"\n\n");
