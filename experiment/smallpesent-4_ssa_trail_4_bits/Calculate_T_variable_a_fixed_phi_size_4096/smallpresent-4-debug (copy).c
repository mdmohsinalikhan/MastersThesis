...
float p_eta_variance_over_fixations[16];
float variance_of_variances_of_p_eta[16];
float p_eta_means[32][16];
float p_eta_variances[32][16];
float ssacpacitysummary[32][3];
float T_a[32][16];
...
printf("Round:%d\n",ROUND);
	for(i = 0;i < 16; i++) //for all eta
	{
		...
		printf("Practical Variance:%.12f Therorectical Variance: %.12f",p_eta_variances[ROUND][i],mlcapacity[ROUND][0]/(15*16));
		printf("\n");
	}
...
int eta=0;
	...
	{
		T_a_phi = T_a_phi + (distribution[fixation][eta] - 4096*(1/16.0))*(distribution[fixation][eta] - 4096*(1/16.0))/(4096*(1/16.0));
	}
	return T_a_phi;
}
...
float mean = 0.0,variance = 0.0,sum = 0.0;
	...
	T_a_summary[ROUND][1] = 15 + 4096*mlcapacity[ROUND][0]; //theoretical mean
	...
	T_a_summary[ROUND][3] = (2.0/15.0)*(15 + 4096*mlcapacity[ROUND][0])*(15 + 4096*mlcapacity[ROUND][0]); //theoretical variance
...
printf("Printing the summary of T_a\n");
	for(i=0; i < 32; i++)
	{
		printf("Round: %d Exp mean: %.12f Theo mean: %.12f Exp variance: %.12f Theo variance: %.12f\n",i,T_a_summary[i][0],T_a_summary[i][1],T_a_summary[i][2],T_a_summary[i][3]);
	}
}
