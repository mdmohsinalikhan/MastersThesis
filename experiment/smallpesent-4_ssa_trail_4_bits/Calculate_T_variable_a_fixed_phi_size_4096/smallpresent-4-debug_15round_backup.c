...
int main()
{
	...
	for(ROUND = 15;ROUND < 16; ROUND++)
	{
		...
		for(fixations = 0;fixations < 16;fixations++)
		{
				...
				printf("For fixation %02x the T(a) value is: %f\n", fixations, calculate_T_a(fixations)); 
				T_a[fixations] = (float)calculate_T_a(fixations);
		}
...
float calculate_T_a(int fixation)
{
	...
	{
		T_a_phi = T_a_phi + (distribution[fixation][eta] - 3500*(1/16.0))*(distribution[fixation][eta] - 3500*(1/16.0))/(3500*(1/16.0));
	}
	return T_a_phi;
}
